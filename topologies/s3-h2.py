#!/usr/bin/python
# -*- coding: utf-8 -*-


from mininet.topo import Topo
from mininet.net import Mininet
from mininet.log import setLogLevel
from mininet.cli import CLI
from mininet.node import OVSSwitch, Controller, RemoteController

class MastersSwitchTopo(Topo):
    def build(self):
        s1 = self.addSwitch('s1', dpid='1', mac="11:00:00:00:00:11", protocols='OpenFlow13', bw=10) # bw está em Mb/seg
        s2 = self.addSwitch('s2', dpid='2', mac="11:00:00:00:00:12", protocols='OpenFlow13', bw=10)
        s3 = self.addSwitch('s3', dpid='3', mac="11:00:00:00:00:13", protocols='OpenFlow13', bw=10)

        h1 = self.addHost('h1', mac="00:00:00:00:00:01", ip="10.0.0.1/12")
        h2 = self.addHost('h2', mac="00:00:00:00:00:02", ip="10.0.0.2/12")

        # Adiciona hosts aos switches
        self.addLink(
            node1 = h1,
 	        node2 = s1,
 	        port1 = 1,
 	        port2 = 1
        ) # link A
        self.addLink(
            node1 = h1,
 	        node2 = s2,
 	        port1 = 2,
 	        port2 = 1
        ) # link B
        self.addLink(
            node1 = h2,
            node2 = s3,
            port1 = 1,
            port2 = 3
        ) # link F
        self.addLink(
            node1 = h2,
            node2 = s2,
            port1 = 2,
            port2 = 2
        ) # link E

        # Adiciona links entre os switches
        self.addLink(
            node1 = s1,
            node2 = s3,
            port1 = 2,
            port2 = 1
        ) # link C
        self.addLink(
            node1 = s3,
            node2 = s2,
            port1 = 2,
            port2 = 3
        ) # link D


if __name__ == '__main__':
    setLogLevel('info')
    topo = MastersSwitchTopo()
    # c1 = RemoteController('c1', ip='127.0.0.1') #usando RYU
    c1 = RemoteController('c1', ip='0.0.0.0', port=6653) #usando Floodlight

    net = Mininet(topo=topo, controller=c1)
    net.start()
    #net.pingAll()
    CLI(net)
    net.stop()
